...
class SkinConductance(DataCollector):
	...
	def add_data(self, new_data):
				...
				if not self.found and d < self.threshold:
					for j in range(i,0,-1):
						...
						if self.filtered_data[j] - self.filtered_data[j-1] > 0:
							if not self.responses[-1]: #and j - self.responses[-2][0] > self.min_freq:
		...
		#if l_resp2 > l_resp:
		#	print self.responses[l_resp:l_resp2]
...
#		t=time.time()
#		if len(self.data) > self.filter_r*2+1:
#			if len(self.filtered_data) <= self.filter_r:
#				self.filtered_data[:] = []
#				self.filtered_data = [mean(self.data[0:2*self.filter_r+1])]*(self.filter_r+1)
#			for i in range(len(self.filtered_data), len(self.data)-self.filter_r-1):
#				self.filtered_data.append(mean(self.data[i-self.filter_r:i+self.filter_r+1]))
#		print time.time()-t
